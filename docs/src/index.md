...
`FeaturedGraph` support various graph representations.
...
FeaturedGraph(
	Undirected graph with (#V=4, #E=5) in adjacency matrix <Matrix{Int64}>,
)
```
...
FeaturedGraph(
	Undirected graph with (#V=4, #E=5) in adjacency list <Vector{Vector{Int64}}>,
)
```

It supports `SimpleGraph` from LightGraphs as well.
